...
效果演示：![演示图片](https://i.cdncf.xyz/66ca2329af38f176fa68c6cce4b6273f.png)
 以下是全部程序讲解：
...
def check_wechat_messages(hw):
    bbb = hw.GetChildren()#获取已显示的联系人列表
    while not we.Exists(0):
        time.sleep(0.5)
    for chatMsg in bbb:#遍历这些人的名字 name->chatMsg.Name
        print(chatMsg.Name)
        if "条新消息" in chatMsg.Name: #如果有新消息会这么显示：小a1条新消息 /小b已置顶1条新消息
            Chatmsg_name = chatMsg.Name
            if "已置顶" in Chatmsg_name:
                Chatmsg_name = Chatmsg_name.replace("已置顶","")#删去已置顶 小b已置顶1条新消息->小b1条新消息
            match = re.match(r'(.+?)(\d+)条新消息', Chatmsg_name)#正则表示法获取新消息的发送人名及新消息数量
            if match:
